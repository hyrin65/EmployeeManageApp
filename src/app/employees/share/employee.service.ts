...
export class EmployeeService {
    ...
    postEmployee(emp: Employee) {
        ...
        return this.http.post('http://localhost:57733/api/Employees', body, options)
            .map(x => x.json());
    }
    getEmployeeList() {
    ...
    putEmployeeEdit(id, emp: Employee) {
        ...
        return this.http.put('http://localhost:57733/api/Employees/' + id, body, options)
            .map(x => x.json());
    }
    deleteEmployee(id) {
        return this.http.delete('http://localhost:57733/api/Employees/' + id)
            .map(x => x.json());
    }
    getEmployeeListById(id) {
        this.http.get('http://localhost:57733/api/Employees/' + id)
            .map((data: Response) => {
                return data.json() as Employee;
            }).toPromise().then(x => {
